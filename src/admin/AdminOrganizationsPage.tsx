...
import { createOrganizationColumns } from "./OrganizationColumns";
...
export const AdminOrganizationsPage: React.FC<AdminOrganizationsPageProps> = ({
  ...
  const [organizations, setOrganizations] =
    useState<Organization[]>(MOCK_ORGANIZATIONS);
  ...
  const handleDeleteOrganization = (orgId: string) => {
    ...
    if (orgToDelete?.is_primary) {
      alert(
        "Cannot delete the primary organization. Please set another organization as primary first.",
      );
      return;
    }
    if (window.confirm("Are you sure you want to delete this organization?")) {
      setOrganizations(
        organizations.filter((org) => org.organization_id !== orgId),
      );
  ...
  const handleSetPrimary = (orgId: string) => {
    if (
      window.confirm(
        "Are you sure you want to set this as the primary organization? This will handle all main operations and settings.",
      )
    ) {
      setOrganizations(
        organizations.map((org) => ({
