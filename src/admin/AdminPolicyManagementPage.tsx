...
import { AddPolicyGroupModal } from "./AddPolicyGroupModal";
import { AddPolicyPermissionModal } from "./AddPolicyPermissionModal";
...
import { createPolicyGroupColumns } from "./PolicyGroupColumns";
import { createPolicyPermissionColumns } from "./PolicyPermissionColumns";
...
const MOCK_POLICY_GROUPS: PolicyGroup[] = [
  ...
  },
  {
    ...
    section: "Contact",
    description:
      "then user can read all patients demographic and other info (not including demographics)",
    key: "pt-rd",
    created_at: "2024-01-15T10:00:00Z",
    updated_at: "2024-01-15T10:00:00Z",
    status: "active",
  },
  {
    ...
    action_short_code: "vw",
    description:
      "the user can view the list of roles, permissions, policy groups",
    key: "ra-vw",
    created_at: "2024-01-15T10:00:00Z",
    updated_at: "2024-01-15T10:00:00Z",
    status: "active",
...
export const AdminPolicyManagementPage: React.FC<
  AdminPolicyManagementPageProps
> = ({ onNavigate }) => {
  const { theme } = useTheme();
  const [activeTab, setActiveTab] = useState<"policy-groups" | "permissions">(
    "policy-groups",
  );

  // Policy Groups State
  const [isPolicyGroupModalOpen, setIsPolicyGroupModalOpen] = useState(false);
  const [policyGroups, setPolicyGroups] =
    useState<PolicyGroup[]>(MOCK_POLICY_GROUPS);
  const [editingPolicyGroup, setEditingPolicyGroup] =
    useState<PolicyGroup | null>(null);

  // Permissions State
  const [isPermissionModalOpen, setIsPermissionModalOpen] = useState(false);
  const [permissions, setPermissions] =
    useState<PolicyPermission[]>(MOCK_PERMISSIONS);
  const [editingPermission, setEditingPermission] =
    useState<PolicyPermission | null>(null);
  ...
  const handleAddPolicyGroup = (data: PolicyGroupFormData) => {
    ...
    if (editingPolicyGroup) {
      ...
      setPolicyGroups(
        policyGroups.map((p) =>
          p.policy_id === editingPolicyGroup.policy_id ? updated : p,
        ),
      );
  ...
  const handleAddPermission = (data: PolicyPermissionFormData) => {
    if (editingPermission) {
      ...
      setPermissions(
        permissions.map((p) =>
          p.perm_id === editingPermission.perm_id ? updated : p,
        ),
      );
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto px-8 py-6">
          <Tabs
            value={activeTab}
            onValueChange={(value) =>
              setActiveTab(value as "policy-groups" | "permissions")
            }
            className="w-full"
          >
            <TabsList
              className={`mb-4 ${theme === "dark" ? "bg-zinc-900" : "bg-gray-100"}`}
            >
            ...
            <TabsContent value="policy-groups" className="mt-0">
              ...
              >
                <div className="mb-4">
                  ...
                  >
                    Define policy categories and actions for role-based access
                    control
                  </p>
                </div>
