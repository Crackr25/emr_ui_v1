...
import { createUserAssignmentColumns } from "./UserAssignmentColumns";
...
export const AdminRoleUsersPage: React.FC<AdminRoleUsersPageProps> = ({
      ...
      users.map((user) => {
        if (user.user_id === userId) {
          if (currentlyAssigned) {
            // Remove role
            console.log(
              `🔄 Removing role "${selectedRole.role_name}" from user: ${user.email}`,
            );
            return {
              ...user,
              role_id: undefined,
            ...
            // Assign role
            console.log(
              `✅ Assigning role "${selectedRole.role_name}" to user: ${user.email}`,
            );
            return {
              ...user,
  ...
  const assignedCount = users.filter(
    (u) => u.role_id === selectedRole.role_id,
  ).length;
    ...
    >
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div
                className={`h-6 w-px ${theme === "dark" ? "bg-zinc-800" : "bg-gray-200"}`}
              />
              <div>
                <h1
                ...
                >
                  {assignedCount} user{assignedCount !== 1 ? "s" : ""} currently
        ...
        <div className="flex-1 overflow-auto px-8 py-6">
          {/* Role Info Card */}
          <div
            className={`${theme === "dark" ? "bg-zinc-900 border-zinc-800" : "bg-white border-gray-200"} border rounded-lg p-4 mb-6`}
          >
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <p
                  className={`text-sm font-medium mt-1 ${theme === "dark" ? "text-white" : "text-gray-900"}`}
                >
              ...
              <div>
                ...
                <p
                  className={`text-sm mt-1 ${theme === "dark" ? "text-zinc-300" : "text-gray-700"}`}
                >
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  {selectedRole.requires_npi && (
                    <span
                      className={`text-xs px-2 py-0.5 rounded ${theme === "dark" ? "bg-blue-950/30 text-blue-400" : "bg-blue-50 text-blue-700"}`}
                    >
                      NPI Required
                    </span>
                  )}
                  {selectedRole.requires_license && (
                    <span
                      className={`text-xs px-2 py-0.5 rounded ${theme === "dark" ? "bg-purple-950/30 text-purple-400" : "bg-purple-50 text-purple-700"}`}
                    >
                      License Required
                    </span>
                  )}
          ...
          >
            <div className="mb-4">
              ...
              >
                Click "Assign" to assign this role to a user, or "Assigned" to
                remove the role
              </p>
            </div>
